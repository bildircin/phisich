...
function Addairdrop() {
    ...
    const [addAirdrops] = useAddAirdropMutation();
    ...
    const getAirdropIndex = async () => {
        const web3 = new Web3(info.readOnlyApi)
        const airdropContract = new web3.eth.Contract(airdropAbi, info.airdropContractAddress)
        const airdropIndex = await airdropContract.methods.getAirdropLength().call()
        return airdropIndex;
    }

    const getAirdropAddress = async () => {
        const web3 = new Web3(info.readOnlyApi)
        const airdropContract = new web3.eth.Contract(airdropAbi, info.airdropContractAddress)
        const indexOfAirdrop = await getAirdropIndex();
        const que = Number(indexOfAirdrop) - 1
        const airdropAddress = await airdropContract.methods.getAirdrop(que).call()
        ...
        try {
            airdropContract.methods.create(a, b, c, d, e, f).send({ from: wallet }, function (error, transactionHash) {
                if (!error) {
                    ...
                    waitForReceipt(transactionHash, async function (receipt) {
                        if (receipt.status) {
                            toast.success('Transaction confirmed!')
                            const contract_address = await getAirdropAddress()
                            const dbData = {
                                contract_address: contract_address,
                                length: "1",
                                tree: merkleProofs,
                                hold_token: holdTokenInput,
                                reward_token: rewardTokenInput,
                                single_reward: e,
                                total_reward: c,
                                reward_token_decimal: dbObj.reward_token_decimal,
                                reward_token_symbol: dbObj.reward_token_symbol,
                                hold_token_symbol: dbObj.hold_token_symbol,
                                minimum_hold: dbObj.minimum_hold,
                                hold_token_decimal: dbObj.hold_token_decimal
                            }
                            addAirdrops(dbData).unwrap()
                                .then((payload) => payload.error ? console.log(payload.error) : toast.success('Airdrop Added'))
                                .catch((error) => console.log(error))
                            //rewardTokenInput, merkelRoot, totalReward, holdTokenInput, singleReward, minimumHold
                        } else {
    ...
    const covalentQuery = async (x) => {
        try {
            ...
            const rewardTokenSymbol = await getTokenSymbol(rewardTokenInput)
            const holdTokenSymbol = await getTokenSymbol(holdTokenInput)
            ...
            let tree = [];
            setDbObj({
                reward_token_decimal: rewardTokenDecimal,
                reward_token_symbol: rewardTokenSymbol,
                hold_token_symbol: holdTokenSymbol,
                minimum_hold: minimumHoldAmountInput,
                hold_token_decimal: decimal
            })
            const fixedRewardArr = rewardWD.split('.');
            const fixedReward = fixedRewardArr[0]
            setSingleReward(fixedReward);
            const filteredDataAdresses = filteredData.map(e => e.address)
            filteredDataAdresses.forEach(add => {
                balances.push({
                    addr: add, amount: web3g.web3.eth.abi.encodeParameter(
                        "uint256",
                        fixedReward
                    )
                })
                tree.push({ address: add, amount: fixedReward })
            })
            ...
            setMerkelRoot(mRoot);
            setMerkleProofs(JSON.stringify(tree))
            console.log(tree, JSON.parse(JSON.stringify(tree)), fixedReward)
    ...
    const getRewardTokenDecimal = async (x) => {
        const tokenC = new web3g.web3.eth.Contract(tokenAbi, x);
        const dec = await tokenC.methods.decimals().call();
        return dec;
    }

    const getTokenSymbol = async (x) => {
        const tokenC = new web3g.web3.eth.Contract(tokenAbi, x);
        const dec = await tokenC.methods.symbol().call();
        return dec;
    }
    ...
    return (
        <div className="flex justify-center mt-8 mx-auto h-[75vh] text-[22px]">
